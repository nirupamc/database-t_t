...
interface AdminEmployeesPageClientProps {
  initialEmployees: AdminEmployeeView[];
}
        ...
        {Array.from({ length: 7 }).map((_, rowIndex) => (
          <div key={rowIndex} className="grid grid-cols-8 gap-3">
            {Array.from({ length: 8 }).map((__, colIndex) => (
              <div key={`${rowIndex}-${colIndex}`} className="h-8 animate-pulse rounded-md bg-slate-700/60" />
            ))}
          </div>
        ))}
      </CardContent>
...
function MobileEmployeeCard({ employee }: { employee: AdminEmployeeView }) {
...
export function AdminEmployeesPageClient({ initialEmployees }: AdminEmployeesPageClientProps) {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter((employee) => `${employee.name} ${employee.email}`.toLowerCase().includes(term));
  }, [employees, search]);

  const columns = useMemo<ColumnDef<AdminEmployeeView>[]>(() => [
    {
      id: "avatar",
      header: "",
      cell: ({ row }) => {
    ...
    { accessorKey: "phone", header: "Phone" },
    {
      accessorKey: "role",
      header: "Role",
      cell: ({ row }) => <Badge variant={row.original.role === "Admin" ? "info" : "secondary"}>{row.original.role}</Badge>,
    },
    { id: "assigned", header: "Assigned Candidates", cell: ({ row }) => row.original.performance.totalAssignedCandidates },
    { id: "applications", header: "Total Applications Submitted", cell: ({ row }) => row.original.performance.totalApplicationsSubmitted },
    { id: "placements", header: "Placements", cell: ({ row }) => row.original.performance.totalPlacements },
        ...
        <>
          ...
          {filteredEmployees.length === 0 && (
            <Card className="border-slate-700 bg-[#1f2937]">
              <CardContent className="py-12 text-center text-slate-300">No employees match this search.</CardContent>
            </Card>
          )}
        </>
      )}
